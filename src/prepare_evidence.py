...
def extract_features_for_evidence(data_path, output_path, data_name_X, encoder='CLIP', choose_encoder_version='ViT-L/14', choose_gpu=1, batch_size=256, num_workders=1, shuffle = False):
...
def re_rank_verite(data_path, data_name, output_path, encoder='CLIP', choose_encoder_version='ViT-L/14'):
    ...
    verite_image_embeddings = pd.DataFrame(verite_image_embeddings, index=[str(x) for x in range(896)]).T
    verite_text_embeddings = pd.DataFrame(verite_text_embeddings, index=[str(x) for x in range(896)]).T
